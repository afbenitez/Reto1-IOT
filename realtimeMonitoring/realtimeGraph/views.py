...
class RemaView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        return context
"""
Se declara la consulta para tomar los eventos surgidos en una fecha y hora dada
La respuesta tiene esta estructura:
{
    "data": [
        {
...
def get_event(request, **kwargs):
    ...
    start_ts = int(startDate.timestamp() * 1000000)
    end_ts = int(endDate.timestamp() * 1000000)
        
    dataForDateTime = Data.objects.filter(time__range=[start_ts,end_ts])
    responseData =dataForDateTime.values()
    return JsonResponse(list(responseData), safe=False)
